...
def read_config_file(Config_Yaml_File):
...
def RecordPreferences(config_array):
    ...
    if os.name == 'nt':
        ...
        slash = '/'
    if config_array[19][-1] != slash:
        outfile_whole = config_array[19] + slash + config_array[18]
    else:
        outfile_whole = config_array[19] + config_array[18]
...
def Write_Doc(config_array, out_data):
    if os.name == 'nt':
        slash = '\\'
    else:
        slash = '/'
    out_doc_name = config_array[19] + config_array[18]
    with open(out_doc_name, 'w', newline='') as csvfile:
